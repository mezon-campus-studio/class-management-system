...
export const ClassDiagram = () => {
  ...
  if (isLoading || !data) return <div className="p-10 text-center animate-pulse text-slate-400 font-medium">Đang tải dữ liệu...</div>;

  const handleSeatClick = async (side: "left" | "right", row: number, col: number) => {
    const studentAtSeat = data.seats.find(s => s.side === side && s.row === row && s.column === col);
    ...
    if (mode === "attendance" && studentAtSeat?.id) {
      ...
      await classDiagramAPI.updateAttendance(studentAtSeat.id, nextStatus[studentAtSeat.status] || "present");
      refresh();
    }
    ...
    <div className="space-y-6 select-none max-w-7xl mx-auto p-2 md:p-4 bg-white min-h-screen">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 bg-slate-50 p-2 rounded-xl border border-slate-200">
        ...
        <div className="flex gap-2 overflow-x-auto no-scrollbar py-1">
          <Badge color="bg-slate-100 text-slate-600 border-slate-200" label="Sĩ số" val={data.totalStudents} />
          <Badge color="bg-green-50 text-green-700 border-green-200" label="Có mặt" val={data.presentCount} />
          <Badge color="bg-yellow-50 text-yellow-700 border-yellow-200" label="Vắng phép" val={data.excusedCount} />
          <Badge color="bg-red-50 text-red-600 border-red-200" label="Không phép" val={data.unexcusedCount} />
        </div>
      </div>

      {/* 2. KHU VỰC GIẢNG ĐƯỜNG (BÀN TRÁI - BẢNG MỎNG Ở GIỮA) */}
      <div className="relative flex items-center h-16 border-b-2 border-slate-100 pb-4">
        {/* Bàn giáo viên bên trái */}
        <div className="bg-yellow-400 px-5 py-3 rounded-lg font-bold text-slate-800 shadow-sm border-b-4 border-yellow-600 text-[10px] md:text-xs whitespace-nowrap">
          BÀN GIÁO VIÊN
        </div>

        {/* Bảng đen mỏng nằm giữa 2 dãy lớp (căn theo lối đi) */}
        <div className="absolute left-1/2 -translate-x-1/2 w-48 md:w-64 h-2 bg-slate-800 rounded-full flex items-center justify-center shadow-lg border border-slate-600">
           <span className="text-[8px] text-white/40 font-bold uppercase tracking-[0.5em] whitespace-nowrap">Bảng Đen</span>
        </div>
      </div>
                ...
                <button
                  ...
                  className={`px-3 py-1 rounded-md text-[10px] font-bold border transition-all ${
                    selectedStudentId === m.id 
                      ...
                      : isAssigned ? "bg-slate-100 text-slate-400 border-slate-200" : "bg-white text-slate-600 border-slate-300"
                  }`}
                >
                  {m.name} {isAssigned && "✓"}
                </button>
      ...
      <div className={`grid grid-cols-[1fr_auto_1fr] gap-4 md:gap-10 pt-4 ${mode !== 'view' ? 'cursor-crosshair' : ''}`}>
        
        {/* Dãy Trái */}
        <div className="flex flex-col gap-6 md:gap-8">
           <div className="grid grid-cols-4 gap-2">
              {firstRows.map(r => cols.map(c => (
                <div key={`L-${r}-${c}`} onClick={() => handleSeatClick("left", r, c)}>
                  <Seat student={data.seats.find(s => s.side === "left" && s.row === r && s.column === c)} />
                </div>
              )))}
           </div>
           <div className="grid grid-cols-4 gap-2">
        ...
        <div className="flex flex-col gap-6 md:gap-8">
           <div className="grid grid-cols-4 gap-2">
              {firstRows.map(r => cols.map(c => (
                <div key={`R-${r}-${c}`} onClick={() => handleSeatClick("right", r, c)}>
                  <Seat student={data.seats.find(s => s.side === "right" && s.row === r && s.column === c)} />
                </div>
              )))}
           </div>
           <div className="grid grid-cols-4 gap-2">
...
const Badge = ({ color, label, val }: { color: string, label: string, val: number }) => (
  <div className={`px-2 py-1.5 rounded-lg text-[9px] md:text-[10px] font-bold shadow-sm border flex items-center gap-1.5 whitespace-nowrap ${color}`}>
